...
// ---------------------------------------------------------------------------
// No Executions (RULES v2.3): teleport-captures may not target royals or Q-slots
// ---------------------------------------------------------------------------
describe('No Executions: Wraith teleport-capture bans on royals and Q-slot pieces', () => {
  function teleportCapturesTo(turns: Turn[], to: string): Turn[] {
    return turns.filter(t =>
      t.primary.type === 'teleport' &&
      (t.primary as TeleportMove).to === sq(to) &&
      (t.primary as TeleportMove).isCapture);
  }

  it('cannot teleport-capture the Herald', () => {
    const state = makeState([
      { slot: 'K', color: 'W', at: 'a1' },
      { slot: 'Q', color: 'W', at: 'd1' }, // Wraith
      { slot: 'K', color: 'B', at: 'e8' },
      { slot: 'Q', color: 'B', at: 'd8' }, // enemy Herald
      { slot: 'P', color: 'B', at: 'h6' }, // off the Wraith's queen lines from d1
    ], { armies: { W: 'Veil', B: 'Accord' }, essence: { W: 2, B: 0 } });
    const turns = legalTurns(state);
    expect(teleportCapturesTo(turns, 'd8').length).toBe(0); // Herald protected
    expect(teleportCapturesTo(turns, 'h6').length).toBe(1); // ordinary pieces are fair game
  });

  it('cannot teleport-capture any Q-slot piece (Shade, Apex, Queen, mirror Wraith)', () => {
    for (const army of ['Phantom', 'Wild', 'Crown', 'Veil'] as const) {
      const state = makeState([
        { slot: 'K', color: 'W', at: 'a1' },
        { slot: 'Q', color: 'W', at: 'd1' },
        { slot: 'K', color: 'B', at: 'e8' },
        { slot: 'Q', color: 'B', at: 'b5' },
      ], { armies: { W: 'Veil', B: army }, essence: { W: 2, B: 2 } });
      const turns = legalTurns(state);
      expect(teleportCapturesTo(turns, 'b5').length).toBe(0);
    }
  });

  it('queen-line captures of a Q-slot piece remain legal', () => {
    // Wraith d1; enemy Herald d8 on the open d-file — the slide capture is allowed.
    const state = makeState([
      { slot: 'K', color: 'W', at: 'a1' },
      { slot: 'Q', color: 'W', at: 'd1' },
      { slot: 'K', color: 'B', at: 'f8' },
      { slot: 'Q', color: 'B', at: 'd8' },
    ], { armies: { W: 'Veil', B: 'Accord' }, essence: { W: 2, B: 0 } });
    const turns = legalTurns(state);
    expect(hasStdMove(turns, 'd1', 'd8')).toBe(true);
  });
});
